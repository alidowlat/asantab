...
from services.models import ServiceVisit
...
class VisitListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['visit_list'] = (
            ServiceVisit.objects.filter(user=request.user).select_related('service').order_by('-created_at')
        )
...
def dashboard_visit_list_partial(request):
    visit_list = ServiceVisit.objects.filter(user=request.user).select_related('service').order_by('-created_at')
